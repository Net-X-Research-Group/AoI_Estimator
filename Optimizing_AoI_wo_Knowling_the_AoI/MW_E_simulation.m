% This function simulates the Max-Weight (MW) scheduling policy with estimation in a network
...
function [actual_throughput,optimal_cost] = MW_E_simulation(K,T,M,h1,z1,Delay,num_iterations,set_Policy,N)
...
for iteration=1:1:num_iterations
    ...
    for slot_index=1:1:K-1      
        ...
        % Renew tauD_hat at lastslot with delayed feedback
        for i=1:1:M
            if slot_index > 2*Delay+1
                if d(i,slot_index-2*Delay)>d(i,slot_index-2*Delay-1)
                    phistory =1;
                    for d_index = d(i,slot_index-2*Delay)+1:1:d(i,slot_index)
                        phistory = phistory*(1-pD(i))^transmissiontime(i,d_index);
                    end
                    tauD_hat_lastslot(i) = tauD_hat_lastslot(i) + ...
                        (tauD_history(i,slot_index-Delay)-tauD_hat_history(i,d(i,slot_index-2*Delay)))*(1-(1-pD(i))^transmissiontime(i,d(i,slot_index-2*Delay)))*phistory + ...
                        (tauD_history(i,slot_index-Delay)-tauD_history(i,slot_index-Delay-1))*((1-pD(i))^transmissiontime(i,d(i,slot_index-2*Delay)))*phistory;          
                end
            end
        end
        ...
        for i=1:1:M
            if slot_index - t_star(i) <= 2*Delay
                if c(i)==1
                ...
                tauD_hat_new_feedback =  slot_index - h_history(i,slot_index);
                tauD_hat(i) = tauD_hat_new_feedback;
            end
        end
